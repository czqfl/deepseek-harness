...
import { DeepSeekBalanceService } from '@deepseek-ai/dsh-deepseek-balance'
...
export function apply(ctx: Context, config: Config): void {
  ...
  // The balance Remote reads the same live options + credential resolution as
  // chat requests, so the page always reflects the configured account.
  ctx.plugin(DeepSeekBalanceService, {
    resolveConnection: async () => ({
      baseURL: options().baseURL,
      apiKey: await resolveApiKey(options()),
    }),
  })
