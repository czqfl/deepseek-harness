...
async function bench(isLoopback = true) {
  ...
  new TestRemote(ctx)
  // The section's balance query injects the generated namespace; no spec
  // drives it, so a stub satisfies the service edge.
  ctx.provide('remote.deepSeekBalance', {
    query: vi.fn(async () => ({ ok: true, value: { ok: false, code: 'fetch-failed', message: 'unused' } })),
  } as never)
...
describe('ui-settings-models apply', () => {
  it('declares the services it uses', () => {
    expect(inject).toEqual(['slots', 'locale', 'connection', 'remote', 'remote.deepSeekBalance'])
  })
