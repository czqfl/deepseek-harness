 ...
 */
export const inject = ['slots', 'locale', 'connection', 'remote', 'remote.deepSeekBalance']
...
export function apply(ctx: ClientContext): void {
  ...
  // The balance Remote result carries carrier failures in the error branch;
  // the section's callback unwraps them into the business failure union so the
  // card renders one failure shape regardless of where it happened.
  const queryBalance = async (): Promise<DeepSeekBalanceResult> => {
    const remote = await ctx.remote.deepSeekBalance.query()
    return remote.ok ? remote.value : {
      ok: false,
      code: 'fetch-failed',
