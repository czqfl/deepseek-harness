...
import type { DeepSeekBalanceResult, IApiClient } from '@deepseek-ai/dsh-api-remotes/client'
import { Button, IconPlusOutline16, Modal } from '@deepseek-ai/dsh-client-ui-primitives'
import type { SnapshotSelectorHook } from '@deepseek-ai/dsh-client-web-react'
import { CustomProviderCard } from './CustomProviderCard.tsx'
import { DeepSeekBalanceCard } from './DeepSeekBalanceCard.tsx'
...
export interface ModelsSectionInjected {
  ...
  /** Query the official DeepSeek account balance (unwrapped business result); absent outside the assembled page. */
...
function Loaded({ injected }: { injected: ModelsSectionInjected }): ReactNode {
  ...
  return (
    <div className={styles['section']}>
      ...
      {/* The official-DeepSeek balance card rides the page only once that
          provider has a configured key and the section received the query
          verb; the query itself resolves the key per call, so a later edit is
          reflected on the next query. */}
      {injected.queryBalance !== undefined
        && state.rows.some(row =>
          row.entry.provider === 'deepseek-official' && row.credential?.configured === true)
        ? <DeepSeekBalanceCard queryBalance={injected.queryBalance} t={t} />
        : null}
