...
import type { UsageBuckets } from '../contract/chat-nodes.ts'
...
/** A non-negative finite count from an unknown usage value, or 0. */
function bucketCount(value: unknown): number {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : 0
}

/**
 * Sum the disjoint provider-usage buckets over one scope's assistant nodes.
 * Each node's `usage` is the durable `TokenUsage` (uncached `inputTokens`,
 * `outputTokens`, optional cache buckets); missing or malformed fields count
 * zero, so a scope with no provider-reported usage folds to all zeros.
 * @param usages - the assistant nodes' usage values.
 * @returns summed disjoint buckets.
 */
export function foldUsage(usages: readonly unknown[]): UsageBuckets {
  const totals = { uncachedInputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 }
  for (const usage of usages) {
    if (typeof usage !== 'object' || usage === null) continue
    const value = usage as UsageBucketLike
    totals.uncachedInputTokens += bucketCount(value.inputTokens)
    totals.outputTokens += bucketCount(value.outputTokens)
    totals.cacheReadTokens += bucketCount(value.cacheReadTokens)
    totals.cacheWriteTokens += bucketCount(value.cacheWriteTokens)
  }
  return totals
}
