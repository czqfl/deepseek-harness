...
import { deriveTurnMetrics, foldUsage } from '../chat/turn-metrics.ts'
...
function tailData(context: ConversationNodeContext<TurnTailState>): TurnTailChatData | null {
  ...
  const usage = foldUsage(finalized.map(candidate => candidate.finalNode.usage))
  const usageFolded = usage.uncachedInputTokens > 0 || usage.outputTokens > 0
