...
import { assistantStepReading, deriveTurnMetrics, foldUsage } from '../src/client/chat/turn-metrics.ts'
...
describe('foldUsage', () => {
  it('sums the disjoint buckets across nodes and defaults absent cache buckets to zero', () => {
    const folded = foldUsage([
      { inputTokens: 10, outputTokens: 5, cacheReadTokens: 90 },
      { inputTokens: 20, outputTokens: 8, cacheReadTokens: 0, cacheWriteTokens: 2 },
    ])
    expect(folded).toEqual({ uncachedInputTokens: 30, outputTokens: 13, cacheReadTokens: 90, cacheWriteTokens: 2 })
  })

  it('counts malformed and missing usage as zero', () => {
    expect(foldUsage([null, 'weird', {}, { inputTokens: Number.NaN, outputTokens: -1 }]))
      .toEqual({ uncachedInputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 })
    expect(foldUsage([])).toEqual({ uncachedInputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 })
  })
})
