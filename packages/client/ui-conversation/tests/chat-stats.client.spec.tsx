...
describe('StatsLine', () => {
  ...
  it('renders the grouped stats row and hides a brand-new empty session', () => {
    ...
    // from the projection, so paging the window cannot change them. The cost
    // group prices the projection under the default DeepSeek CNY pricing.
    expect(view.container.textContent).toBe('1 turns · 1 steps| Cache hit 90%| Input 100 tok · Output 5 tok| Cost ¥0.000105')
  ...
  it('reveals the full line in a delayed hover tooltip only while the row is clipped', () => {
    ...
    expect(view.container.querySelector('[role="tooltip"]')?.textContent)
      .toBe('1 turns · 1 steps | Cache hit 90% | Input 100 tok · Output 5 tok | Cost ¥0.000105')
  })
  ...
  it('takes every stats label from the active locale', () => {
    ...
    expect(view.container.textContent)
      .toBe('1 轮 · 1 步| LLM 3.8s| 首 token 平均 0.8s · 20 tok/s| 缓存命中 90%| 输入 100 tok · 输出 5 tok| 费用 ¥0.000105')
  })
  ...
  it('keeps durable token groups after the visible step window is empty', () => {
    ...
    expect(view.container.textContent)
      .toBe('Cache hit 90%| Input 100 tok · Output 5 tok| Cost ¥0.000105')
  })
  ...
  it('renders whole-session counts from the sessionStats projection over the paged window', () => {
    ...
    expect(view.container.textContent)
      .toBe('10 turns · 89 steps| Cache hit 90%| Input 100 tok · Output 5 tok| Cost ¥0.000105')
  })
  ...
  it('keeps the counts group over an empty visible window when the projection carries totals', () => {
    ...
    expect(view.container.textContent)
      .toBe('7 turns · 44 steps| Cache hit 90%| Input 100 tok · Output 5 tok| Cost ¥0.000105')
  })
  ...
  it('renders whole-log wall times and speeds from the projection, not the loaded window', () => {
    ...
    expect(view.container.textContent).toBe(
      '200 turns · 200 steps| LLM 1m40s · Tool call 1m2s| TTFT avg 0.8s · 20 tok/s| Cache hit 90%| Input 100 tok · Output 5 tok| Cost ¥0.000105',
    )
  })
  ...
  it('omits cache hit when nothing was billed on the input side', () => {
    ...
    expect(view.container.textContent).toBe('1 turns · 1 steps| Input 0 tok · Output 7 tok| Cost ¥0.000056')
  })
  ...
  it('includes cache writes in billed input and the cache-hit denominator', () => {
    ...
    expect(view.container.textContent)
      .toBe('1 turns · 1 steps| Cache hit 45%| Input 200 tok · Output 7 tok| Cost ¥0.000121')
  })
